...
class PatternRecognizer:
    ...
    async def extract_patterns(
        ...
        executions = (
            await self.db["agent_memory"]
            .find({"agent_name": agent_name, "timestamp": {"$gte": cutoff_iso}})
            .to_list(1000)
        )
        ...
        patterns["time_patterns"] = {
            str(h): {
                "count": v["count"],
                "success_rate": (
                    (v["success"] / v["count"] * 100) if v["count"] > 0 else 0
                ),
            }
        ...
        for i in range(0, len(executions), window_size):
            ...
            window_avg_duration = statistics.mean(
                [e.get("duration_ms", 0) for e in window]
            )
            ...
            patterns["performance_trends"].append(
                {
                    ...
                    "success_rate": (
                        (window_success / len(window) * 100) if window else 0
                    ),
    ...
    async def detect_anomalies(
        self, agent_name: str, lookback_hours: int = 24
    ) -> List[Dict[str, Any]]:
...
class StrategyAdapterAdvanced:
    ...
    async def adapt_strategy(
        self, agent_name: str, current_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if (
            patterns.get("avg_duration_ms", 0) > 1000
            and len(patterns.get("common_inputs", [])) > 2
        ):
...
class CostOptimizer:
    ...
    async def analyze_token_costs(
        ...
        records = (
            await self.db["prompt_efficiency"]
            .find({"agent_name": agent_name, "timestamp": {"$gte": cutoff_iso}})
            .to_list(1000)
        )
    ...
    async def get_cost_recommendations(self) -> List[Dict[str, Any]]:
        ...
        # Sort by cost
        recommendations.sort(key=lambda x: x.get("total_cost_haiku", 0), reverse=True)
