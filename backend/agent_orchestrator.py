...
class AgentOrchestrator:
    ...
    async def execute_workflow(
        ...
        while pending_tasks or (completed_tasks and len(completed_tasks) < len(tasks)):
            ...
            if parallel:
                ...
                results = await asyncio.gather(
                    *[self._execute_task(task, on_task_complete) for task in ready_tasks],
        ...
        logger.info(f"Workflow complete: {len(completed_tasks)}/{len(tasks)} tasks succeeded")
    ...
    async def _execute_task(
        ...
        strategy = await self.strategy.get_recommended_strategy(task.agent_name, task.context)
        logger.info(f"Task {task.task_id} strategy: {strategy.get('success_rate', 0):.1f}% success")
        ...
        for attempt in range(task.max_retries + 1):
            try:
                ...
                should_retry = await self.memory.should_retry(task.agent_name, str(e), attempt)
                if not should_retry:
                    break
        ...
        logger.error(f"Task {task.task_id} failed after {task.max_retries + 1} attempts: {last_error}")
    ...
    async def analyze_workflow_performance(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "successful_tasks": sum(1 for t in self.executed_tasks.values() if t.status == "completed"),
            "failed_tasks": sum(1 for t in self.executed_tasks.values() if t.status == "failed"),
            "agent_performance": {},
        }
        ...
        for agent_name in agents:
            agent_tasks = [t for t in self.executed_tasks.values() if t.agent_name == agent_name]
            ...
            if durations:
                analysis["agent_performance"][agent_name] = {
                    ...
                    "success_rate": sum(1 for t in agent_tasks if t.status == "completed") / len(agent_tasks) * 100,
                }
