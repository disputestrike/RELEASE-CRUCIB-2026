...
Output: JSON context.""",
    ...
    async def analyze_prompt_efficiency(
        self, agent_name: str, prompt: str, response: str, tokens_used: int
    ) -> Dict[str, Any]:
        """
    ...
    async def get_optimization_suggestions(
        ...
        records = (
            ...
            .find({"agent_name": agent_name, "timestamp": {"$gte": cutoff}})
            .to_list(100)
        )
        ...
        # Analyze patterns
        avg_efficiency = sum(r.get("efficiency_ratio", 1) for r in records) / len(
            records
        )
