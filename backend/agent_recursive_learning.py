...
class AgentMemory:
    ...
    async def get_execution_history(
        ...
        try:
            executions = (
                await self.db[COLLECTION_AGENT_MEMORY]
                .find(query)
                .sort("timestamp", -1)
                .limit(limit)
                .to_list(limit)
    ...
    async def extract_patterns(self, agent_name: str) -> Dict[str, Any]:
        ...
        successes = sum(
            1 for e in executions if e["status"] == ExecutionStatus.SUCCESS.value
        )
        ...
        successful_inputs = [
            e["input_summary"]
            for e in executions
            if e["status"] == ExecutionStatus.SUCCESS.value
        ]
    ...
    async def get_learnings(
        ...
        query = {"agent_name": agent_name, "confidence": {"$gte": min_confidence}}
        ...
        try:
            learnings = (
                await self.db[COLLECTION_AGENT_LEARNINGS]
                .find(query)
                .sort("confidence", -1)
                .to_list(50)
            )
    ...
    async def apply_learning(self, learning_id: str) -> None:
        ...
        try:
            await self.db[COLLECTION_AGENT_LEARNINGS].update_one(
                {"_id": learning_id}, {"$inc": {"applied_count": 1}}
            )
        except Exception as e:
    ...
    async def record_learning_success(self, learning_id: str) -> None:
        ...
        try:
            await self.db[COLLECTION_AGENT_LEARNINGS].update_one(
                {"_id": learning_id}, {"$inc": {"success_count": 1}}
            )
        except Exception as e:
...
class PerformanceTracker:
    ...
    async def get_metrics_summary(
        self, agent_name: str, metric_name: Optional[str] = None, hours: int = 24
    ) -> Dict[str, Any]:
        """Get summary of metrics for an agent."""

        cutoff_time = (datetime.now(timezone.utc) - timedelta(hours=hours)).isoformat()

        query = {"agent_name": agent_name, "timestamp": {"$gte": cutoff_time}}
        ...
        try:
            metrics = (
                await self.db[COLLECTION_AGENT_PERFORMANCE].find(query).to_list(1000)
            )
...
class AdaptiveStrategy:
    ...
    async def should_retry(
        self, agent_name: str, error: str, retry_count: int = 0
    ) -> bool:
