...
class FrontendAgent(BaseAgent):
    ...
    def validate_output(self, result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            raise AgentValidationError(
                f"{self.name}: package.json must be valid JSON: {e}"
            )
        ...
        for field in structure_fields:
            if field not in result["structure"]:
                raise AgentValidationError(
                    f"{self.name}: structure missing field '{field}'"
                )
    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if design_output:
            ...
            if colors:
                ...
                design_info += (
                    f"Secondary Color: {colors.get('secondary', '#808080')}\n"
                )
...
- Do not include markdown fences, prose, comments outside JSON, or placeholder TODO-only files."""
        ...
        logger.info(
            f"PARSED RESULT: files={len(data.get('files', {}))}, structure={bool(data.get('structure'))}"
        )

        # EXPLICIT VALIDATION
        if not data:
            raise AgentValidationError(
                f"{self.name}: parse_json_response returned empty result"
            )

        if not data.get("files"):
            raise AgentValidationError(
                f"{self.name}: No files in parsed JSON: {list(data.keys())}"
            )
