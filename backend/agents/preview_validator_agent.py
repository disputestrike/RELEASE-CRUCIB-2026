...
class PreviewValidatorAgent:
    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Preview validation: {len(critical_issues)} critical, {len(warnings)} warnings")
    ...
    async def _validate_vite_config(self, context: Dict) -> Dict[str, Any]:
        ...
        if "export default" not in config_content and "module.exports" not in config_content:
            return {
                "file": "vite.config.js",
                "critical": True,
        ...
        if self._has_jsx_files(workspace):
            if "@vitejs/plugin-react" not in config_content and "@vite/plugin-react" not in config_content:
                return {
                    "file": "vite.config.js",
                    "critical": True,
    ...
    async def _validate_dependencies_exist(self, context: Dict) -> Dict[str, Any]:
        ...
        for js_file in js_files:
            ...
            for imp in imports:
                ...
                if root_pkg not in all_deps and root_pkg != "react" and root_pkg != "react-dom":
