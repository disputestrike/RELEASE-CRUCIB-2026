    ...
    def validate_output(self, result: Dict[str, Any]) -> bool:
        ...
        for field in frontend_fields:
            if field not in result["frontend"]:
                raise AgentValidationError(
                    f"{self.name}: Missing frontend field '{field}'"
                )
        ...
        for field in backend_fields:
            if field not in result["backend"]:
                raise AgentValidationError(
                    f"{self.name}: Missing backend field '{field}'"
                )
        ...
        for field in database_fields:
            if field not in result["database"]:
                raise AgentValidationError(
                    f"{self.name}: Missing database field '{field}'"
                )
        ...
        for field in deployment_fields:
            if field not in result["deployment"]:
                raise AgentValidationError(
                    f"{self.name}: Missing deployment field '{field}'"
                )
