...
class TestGenerator:
    def _generate(
        self, test_type: str, code: str, language: str, framework: str
    ) -> str:
        ...
        result = _run_async(
            call_llm(
                system_prompt, user_prompt, temperature=0.3, task_type="documentation"
            )
        )
        ...
        logger.warning(
            "LLM returned empty response for %s tests; returning scaffold", test_type
        )
        return _scaffold_tests(test_type, language, framework)

    def generate_unit_tests(
        self, code: str, language: str, framework: Optional[str]
    ) -> "GeneratedTest":
...
class CodeOptimizer:
    def optimize(
        self, code: str, language: str, optimization_type: str
    ) -> Dict[str, Any]:
