...
class DomainDetector:
    ...
    async def detect_domain(self, requirements: str) -> DomainDetectionResult:
        ...
        sorted_domains = sorted(
            matched_domains.items(), key=lambda x: x[1][0], reverse=True
        )
...
class DomainLearner:
    ...
    async def learn_from_build(
        self, domain: str, requirements: str, generated_code: str, success: bool
    ) -> Dict[str, Any]:
        """
        ...
        if domain not in self.learned_domains:
            ...
            if success:
                domain_knowledge.success_rate = (
                    domain_knowledge.success_rate * (domain_knowledge.usage_count - 1)
                    + 1
        ...
        logger.info(
            f"Learned {len(patterns)} patterns, {len(constraints)} constraints, {len(best_practices)} practices"
        )
    ...
    def _identify_constraints(
        self, domain: str, requirements: str, code: str
    ) -> List[Dict[str, Any]]:
        """Identify constraints that were applied"""
        constraints = []
...
class AutonomousDomainAgent:
    ...
    async def analyze_requirements(self, requirements: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "domain_knowledge": (
                primary_knowledge.to_dict() if primary_knowledge else None
            ),
            "secondary_knowledge": [k.to_dict() for k in secondary_knowledge if k],
            "applicable_constraints": (
                primary_knowledge.constraints if primary_knowledge else []
            ),
            "best_practices": (
                primary_knowledge.best_practices if primary_knowledge else []
            ),
            "timestamp": datetime.utcnow().isoformat(),
        }
    ...
    async def apply_domain_constraints(
        self, domain: str, generated_code: str
    ) -> Dict[str, Any]:
        """
