...
class CodeValidator:
    ...
    def validate_javascript(code: str) -> Dict[str, Any]:
        ...
        """
        result = {"is_valid": True, "syntax_valid": True, "errors": [], "warnings": []}
        ...
        try:
            with tempfile.NamedTemporaryFile(mode="w", suffix=".js", delete=False) as f:
                ...
                try:
                    ...
                    logger.warning(
                        "node not available, skipping JavaScript syntax check"
                    )
                finally:
                    os.unlink(f.name)
        except Exception as e:
    ...
    def validate_sql(code: str) -> Dict[str, Any]:
        ...
        if not any(
            kw in code_upper
            for kw in ["CREATE", "INSERT", "SELECT", "UPDATE", "DELETE", "DROP"]
        ):
    ...
    def validate_code(code: str, language: str = None) -> Dict[str, Any]:
        ...
        if not language:
            if any(kw in code for kw in ["import ", "def ", "class ", "async def"]):
                ...
                kw in code for kw in ["function", "const ", "let ", "var ", "import {"]
            ):
                language = "javascript"
