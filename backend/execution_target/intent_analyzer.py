...
class IntentAnalyzer:
    ...
    TARGET_KEYWORDS = {
        ExecutionTarget.FULLSTACK_WEB: [
            "dashboard", "app", "interface", "ui", "real-time", "interactive",
            "web app", "webapp", "frontend", "react", "responsive"
        ],
        ExecutionTarget.NEXTJS_APP: [
            "next.js", "nextjs", "ssg", "ssr", "static generation", "page", "route"
        ],
        ExecutionTarget.MARKETING_STATIC: [
            "landing", "static", "marketing", "brochure", "blog", "content", "documentation"
        ],
        ExecutionTarget.API_BACKEND_FIRST: [
            "api", "backend", "database", "rest", "graphql", "endpoint", "service"
        ],
        ExecutionTarget.AGENTS_AUTOMATION: [
            "agent", "automation", "workflow", "task", "bot", "intelligent"
        ]
    }
    ...
    def analyze(self, user_request: str) -> Dict:
        ...
        ranked = sorted(target_scores.items(), key=lambda x: x[1]["score"], reverse=True)
        ...
        secondary = [t[0] for t in ranked[1:3] if t[0] != primary and t[1]["score"] > 30]
