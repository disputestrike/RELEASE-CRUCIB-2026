...
class TargetLearningSystem:
    ...
    def record_choice(
        ...
        logger.info(
            f"Recorded choice: {user_choice} (suggestion: {suggested_target}, outcome: {outcome})"
        )
    ...
    def _update_confidence_adjustments(self) -> None:
        ...
        for target, stats in by_target.items():
            ...
            logger.info(
                f"Target {target}: {stats['correct']}/{stats['total']} correct (adjustment: {adjustment:+.1f})"
            )
    ...
    def get_learning_stats(self) -> Dict:
        ...
        if not self.choice_history:
            return {"total_choices": 0, "accuracy": 0, "target_accuracies": {}}
