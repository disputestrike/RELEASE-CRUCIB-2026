...
BUILD_STRUCTURES = {
    "fullstack": {
        "passes": [
            ...
            {
                "name": "more_pages_and_contexts",
                "desc": "More pages + contexts + hooks",
                "files": [
                    ...
                    "/src/hooks/useFetch.ts          — useFetch<T>(url): { data, loading, error, refetch }",
                    "/src/lib/api.ts                 — API client: baseURL, axios instance, interceptors for auth token",
                    "/shared/types.ts                — TypeScript interfaces: User, Product, NavLink, Feature, Testimonial, PricingPlan, ContactForm",
                ],
            },
            {
                "name": "backend_and_infra",
                "desc": "Full Express backend + database + Docker + CI/CD + README",
                "files": [
                    "/server/index.ts                — Express app: import routes, middleware; cors, helmet, json; listen port 5000",
                    "/server/routes/index.ts         — import all routers, mount at /api/health /api/auth /api/users /api/items",
                    "/server/routes/auth.ts          — POST /login (JWT), POST /signup, POST /logout, GET /me",
                    "/server/routes/items.ts         — GET /items, GET /items/:id, POST /items, PUT /items/:id, DELETE /items/:id",
                    "/server/middleware/auth.ts      — verifyToken middleware: decode JWT, attach req.user",
                    "/server/middleware/validate.ts  — validateBody(schema) middleware using zod",
                    "/server/services/authService.ts — hashPassword, comparePassword, generateToken, verifyToken",
                    "/server/services/itemService.ts — getAll, getById, create, update, delete with mock DB",
                    "/server/models/user.ts          — User type + mock users array (replace with DB later)",
                    "/server/models/item.ts          — Item type + mock items array",
                    "/database/schema.sql            — CREATE TABLE users, items, sessions with indexes",
                    "/docker-compose.yml             — services: frontend (3000), backend (5000), postgres (5432)",
                    "/.github/workflows/deploy.yml   — CI: checkout, install, build, test; CD: deploy to Railway on main push",
                    "/README.md                      — # Project\n## Setup: npm install, npm run dev\n## Structure: explain folders\n## API: list endpoints\n## Deploy: Railway instructions",
                    "/ideas.md                       — # Project Notes\n## Features built\n## Architecture decisions\n## Future improvements",
                ],
            },
        ]
    },
    ...
    "ai_agent": {
        "passes": [
            ...
            {
                "name": "pages_and_backend",
                "desc": "Agent pages + full backend",
                "files": [
                    ...
                    "/server/index.ts               — Express: middleware, routes setup",
                    "/server/routes/chat.ts         — POST /api/chat: accept message, return simulated AI streaming response",
                    "/server/routes/agents.ts       — GET /api/agents, GET /api/agents/:id, POST /api/agents",
                    "/server/services/aiService.ts  — simulateResponse(prompt, agentConfig): fake typewriter streaming",
                    "/shared/types.ts               — Agent, Message, Conversation, Tool, ChatConfig interfaces",
                    "/database/schema.sql           — agents, conversations, messages, tools tables",
