...
def _critical_runtime_evidence(text: str, tags: List[str]) -> Tuple[bool, List[str]]:
    ...
    if "auth" in tags:
        ...
        if not ok:
            issues.append(
                "Auth-related goal: classify auth as Implemented/Mocked/Stubbed explicitly."
            )
    return len(issues) == 0, issues
...
async def verify_elite_builder_workspace(
    ...
    *,
    ...
    if not (elite_txt or "").strip():
        checks.append(
            {
                "name": "elite_directive",
                "passed": False,
                "reason": "Missing proof/ELITE_EXECUTION_DIRECTIVE.md",
            }
        )
        issues.append(
            "Missing proof/ELITE_EXECUTION_DIRECTIVE.md — elite execution authority not materialized"
        )
    else:
    ...
    if not (dc or "").strip():
        checks.append(
            {
                ...
                "passed": False,
                "reason": "Missing proof/DELIVERY_CLASSIFICATION.md",
            }
        )
        ...
        if not ok_crit:
            checks.append(
                {
                    "name": "critical_feature_proof",
                    "passed": False,
                    "reason": "; ".join(crit_issues),
                }
            )
...
def _check_error_boundaries(workspace_path: str) -> Dict[str, Any]:
    ...
    return {
        "name": "error_boundaries",
        "passed": False,
        "reason": "No Error Boundary component found",
    }
...
def _check_console_errors(workspace_path: str) -> Dict[str, Any]:
                ...
                if f.endswith((".jsx", ".ts", ".tsx", ".js")):
                    ...
                    if text:
                        ...
                        if ".test." in f or ".spec." in f:
                            continue
                        count += text.count("console.error") + text.count(
                            "console.warn"
                        )
    except:
        pass
    ...
    if count > 0:
        return {
            "name": "console_errors",
            "passed": False,
            "reason": f"Found {count} console.error/warn calls",
        }
    return {"name": "console_errors", "passed": True}
        ...
        if os.path.exists(backend_file):
            ...
            if text and ("@app.post" in text or "@app.get" in text):
                ...
                return {
                    ...
                    "passed": False,
                    "reason": "API routes lack error handling (try-catch)",
                }
    except:
        pass
                    ...
                    if text:
                        ...
                        if "execute(f'" in text or 'execute(f"' in text:
                            return {
                                "name": "sql_safety",
                                "passed": False,
                                "reason": "SQL with f-string interpolation detected",
                            }
    except:
        pass
...
def _check_authentication(workspace_path: str) -> Dict[str, Any]:
    ...
    return {
        "name": "authentication",
        "passed": False,
        "reason": "No authentication/authorization patterns found",
    }
...
def _check_hardcoded_secrets(workspace_path: str) -> Dict[str, Any]:
    ...
    if count > 0:
        return {
            ...
            "passed": False,
            "reason": f"Found {count} potential hardcoded secrets",
        }
    return {"name": "hardcoded_secrets", "passed": True}
...
def _check_security_headers(workspace_path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "passed": False,
        "reason": f"Only {found}/5 security headers configured",
    }
