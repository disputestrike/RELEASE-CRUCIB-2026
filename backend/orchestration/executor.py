...
def _stripe_webhook_routes_sketch() -> str:
    return '''"""Stripe webhook handler — signature verification + idempotency (CrucibAI).
1) pip install stripe
2) Set STRIPE_SECRET_KEY and STRIPE_WEBHOOK_SECRET env vars
3) Configure webhook endpoint in Stripe Dashboard → /api/payments/webhook
"""
import os
import stripe
...
async def stripe_webhook(request: Request, stripe_signature: str = Header(None)):
    ...
    if event_type == "checkout.session.completed":
        ...
        # TODO: fulfil order — grant access, update DB, etc.
        pass
...
def _ensure_stripe_router_mounted(workspace_path: str) -> None:
    """Append Stripe router mount to backend/main.py once."""
    rel = "backend/main.py"
...
# CRUCIBAI_STRIPE_ROUTER_MOUNT — Stripe payment routes (backend/stripe_routes.py)
try:
