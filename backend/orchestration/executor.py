...
from .domain_packs import compliance_regulated_intent, multitenant_intent, payment_intent
...
def _paypal_routes_sketch() -> str:
    return '''"""PayPal checkout/subscription routes generated by CrucibAI.
1) Set PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET env vars
2) Configure PayPal webhooks to /api/payments/paypal/webhook
3) Persist capture/subscription records in your production database
"""
import os
...
async def paypal_status():
    return {"provider": "paypal", "configured": _configured()}
...
async def create_order(payload: dict):
    ...
    # Replace with PayPal Orders API call in production.
    return {"paypal_order_id": payload.get("idempotency_key") or "paypal_order_pending"}


@router.post("/capture-order")
async def capture_order(payload: dict):
    if not _configured():
        raise HTTPException(status_code=503, detail="PayPal is not configured")
    return {"ok": True, "paypal_order_id": payload.get("paypal_order_id")}
...
def _ensure_paypal_router_mounted(workspace_path: str) -> None:
    rel = "backend/main.py"
    text = _read_text(workspace_path, rel)
    if not text or "CRUCIBAI_PAYPAL_ROUTER_MOUNT" in text:
        return
    if "FastAPI" not in text or "app =" not in text:
        return
    suffix = """

# CRUCIBAI_PAYPAL_ROUTER_MOUNT - PayPal payment routes (backend/paypal_routes.py)
try:
    import paypal_routes
...
- [ ] PayPal: webhook signature + `paypal_events_processed` idempotency table
...
# Hard-reject patterns: if .py file starts with these, the whole content is garbage
_HARD_REJECT_PREFIXES_PY = (
    "# generated by",
    "# auto-generated",
    "# crucibai",
    "# this file",
...
def _strip_prose_preamble(content: str, rel: str) -> str:
    ...
    if ext not in _CODE_FILE_EXTS:
        return content
    # Hard-reject .py files whose content is clearly markdown/manifest, not Python
    if ext == ".py":
        first_line = content.strip().split("\n")[0].strip().lower()
        if any(first_line.startswith(p) for p in ("# generated by cruci", "```", "---\n", "## ", "### ", "#### ")):
            logger.warning("executor: REJECTED .py file %s — starts with markdown/manifest content", rel)
            return ""  # _safe_write will skip empty content
...
'''
        ...
        elif key == "backend.paypal":
            paypal_py = _paypal_routes_sketch()
            w = _safe_write(workspace_path, "backend/paypal_routes.py", paypal_py)
            if w:
                out_files.append(w)
            _ensure_paypal_router_mounted(workspace_path)
            if "backend/main.py" not in out_files:
                out_files.append("backend/main.py")
            routes_added = [
                {
                    "method": "POST",
                    "path": "/api/payments/paypal/create-order",
                    "description": "PayPal order creation",
                },
                {
                    "method": "POST",
                    "path": "/api/payments/paypal/webhook",
                    "description": "PayPal webhook receiver",
                },
            ]
        ...
        if key == "database.migration":
            ...
            if mt:
                ...
                if w2:
                    out.append(w2)
            if st:
                sql_st = """-- 003_paypal_idempotency_sketch.sql - webhook dedupe
...
"""
                w3 = _safe_write(
                    workspace_path,
                    "db/migrations/003_paypal_idempotency_sketch.sql",
                    sql_st,
                    job_id=job.get("id"),
                )
                if w3:
...
async def handle_deploy(step: Dict, job: Dict, workspace_path: str, **kwargs) -> Dict:
    ...
    if workspace_path:
        if key == "deploy.build":
            goal_lower = (job.get("goal") or "").lower()
            _has_python = any(w in goal_lower for w in ("python", "fastapi", "flask", "django", "backend", "api"))
            if _has_python:
                docker = """# Generated by CrucibAI — Python/FastAPI stack
...
"""
                ...
                docker = """# Generated by CrucibAI — Node/React stack
...
- Third-party APIs (PayPal, OAuth, email, etc.) using placeholder or test keys in `.env.example` until production secrets exist.
...
STEP_HANDLERS = {
    ...
    "backend.paypal": handle_backend_route,
