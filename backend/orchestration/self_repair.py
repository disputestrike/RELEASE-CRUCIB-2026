...
def repair_prose_in_file(workspace_path: str, rel_path: str) -> Dict[str, Any]:
    ...
    return {
        "fixed": True, "file": rel_path, "lines_removed": lines_removed,
        "first_line_after": cleaned.strip().split("\n")[0][:80] if cleaned.strip() else "",
    }
...
def repair_entry_point(workspace_path: str) -> Dict[str, Any]:
    ...
    _safe_write(workspace_path, "src/main.jsx", """import React from 'react';
...
def repair_app_jsx_if_broken(workspace_path: str) -> Dict[str, Any]:
    ...
    if not is_prose and not is_empty:
        ...
        if cleaned != content:
            ...
            return {
                "fixed": True, "file": "src/App.jsx", "action": "stripped_prose",
                "first_line": cleaned.strip().split("\n")[0][:80],
            }
        return {"fixed": False, "reason": "App.jsx appears valid"}

    _safe_write(workspace_path, "src/App.jsx", """import React from 'react';
...
def repair_vite_config(workspace_path: str) -> Dict[str, Any]:
    ...
    _safe_write(workspace_path, "vite.config.js", """import { defineConfig } from 'vite';
...
def repair_index_html(workspace_path: str) -> Dict[str, Any]:
    ...
    _safe_write(workspace_path, "index.html", """<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
async def apply_self_repair(
    ...
    if root_cause in ("prose_in_code", "regenerate_frontend", "jsx_syntax_error") or \
       any("App.jsx" in f for f in affected_files):
    ...
    # 3. Fix package.json
    if root_cause in ("missing_dependencies", "regenerate_package_json") or \
       any("package.json" in f for f in affected_files):
    ...
    # 4. Fix entry point
    if root_cause in ("no_entry_point", "regenerate_entry_point") or \
       not diagnosis.get("has_app_jsx"):
    ...
    logger.info(
        ...
        step_key, root_cause, fixed_count, len(repairs),
    )
    return {
        "repairs": repairs,
