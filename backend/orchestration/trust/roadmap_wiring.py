...
def roadmap_wiring_status() -> List[Dict[str, Any]]:
    return [
        {"id": "github_actions_verify_full", "phase": 5, "status": "wired", "detail": ".github/workflows/ci-verify-full.yml — pytest + uvicorn /api/health + deploy/healthcheck.sh + frontend lint/test:ci"},
        {"id": "observability_workspace_pack", "phase": 4, "status": "wired", "detail": "deploy/observability stubs + docs/OBSERVABILITY_PACK.md on deploy.build when goal matches"},
        {"id": "multiregion_terraform_sketch", "phase": 5, "status": "wired", "detail": "terraform/modules/{aws,gcp,azure}_region_stub + multiregion_sketch when goal matches"},
        {"id": "spec_guardian_engine", "phase": 1, "status": "wired", "detail": "spec_guardian.py + plan spec_guard + run-auto event; strict mode blocks run"},
        {"id": "truthful_multiaxis_scores", "phase": 2, "status": "wired", "detail": "proof scorecard: pipeline vs trust vs spec_compliance vs production_readiness"},
        {"id": "architecture_outline_on_plan", "phase": 2, "status": "wired", "detail": "planner architecture_outline (honest template intent)"},
        {"id": "domain_pack_multitenant_sql_api", "phase": 3, "status": "partial", "detail": "goal-driven 002_multitenancy_rls.sql (live RLS on app_items) + X-Tenant-Slug API + Tenant models; CI tests RLS"},
        {"id": "domain_pack_stripe_idempotency_sql", "phase": 3, "status": "partial", "detail": "003_stripe_idempotency_sketch.sql + stripe_routes webhook sketch"},
        {"id": "domain_pack_compliance_sketch_md", "phase": 3, "status": "wired", "detail": "regulated goal → docs/COMPLIANCE_SKETCH.md + file proof + UI callout; truth_scores factor"},
        {"id": "production_gate_secret_scan", "phase": 5, "status": "wired", "detail": "deploy.build scans workspace; CRUCIBAI_PRODUCTION_GATE_STRICT=1 fails on patterns"},
        {"id": "verification_security_workspace", "phase": 4, "status": "wired", "detail": "verification.security runs migration + main.py + package checks"},
        {"id": "stripe_router_mounted_in_main", "phase": 4, "status": "wired", "detail": "backend.stripe patches main.py with include_router(stripe_routes.router)"},
        {"id": "verification_api_smoke_static_live", "phase": 4, "status": "partial", "detail": "api_smoke: route scan + py_compile; optional CRUCIBAI_API_SMOKE_URL live GET"},
        {"id": "deploy_healthcheck_script", "phase": 5, "status": "wired", "detail": "deploy/healthcheck.sh curls /health"},
