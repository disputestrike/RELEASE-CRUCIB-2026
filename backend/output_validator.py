...
class OutputValidator:
    ...
    def validate_sql_code(code: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if 'CREATE TABLE' not in code_upper and 'INSERT' not in code_upper and 'SELECT' not in code_upper:
    ...
    @staticmethod
    def validate_agent_output(agent_name: str, output: str, expected_format: str) -> Dict[str, Any]:
        """
        ...
        # Check format
        format_valid, format_error = OutputValidator.validate_output_format(output, expected_format)
        ...
        if expected_format == "json":
            ...
            if "import" in output or "def " in output or "class " in output:
                syntax_valid, syntax_error = OutputValidator.validate_python_code(output)
            elif "function" in output or "const " in output or "let " in output:
                syntax_valid, syntax_error = OutputValidator.validate_javascript_code(output)
            else:
                syntax_valid, syntax_error = OutputValidator.validate_python_code(output)
        ...
        if result["is_valid"]:
            ...
            logger.error(f"❌ {agent_name} output validation FAILED: {result['errors']}")
