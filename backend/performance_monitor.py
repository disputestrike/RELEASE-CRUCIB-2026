...
class PerformanceMonitor:
    ...
    def record_error(
        ...
        logger.error(f"[{severity.upper()}] {agent_name}: {error_type} - {error_message}")
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        total_executions = sum(stats["executions"] for stats in self.agent_stats.values())
    ...
    def print_report(self):
        ...
        for agent in slowest:
            print(f"  {agent['agent']}: {agent['avg_time']:.2f}s avg ({agent['executions']} executions)")
        ...
        for agent in most_used:
            print(f"  {agent['agent']}: {agent['executions']} executions ({agent['success_rate']:.1f}% success)")
