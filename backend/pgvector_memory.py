...
class PgVectorMemory:
    ...
    async def initialize(self, db_pool):
        ...
        try:
            ...
            logger.info(
                "✅ pgvector extension initialized and embeddings table created"
            )
    ...
    async def store_embedding(
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                logger.debug(
                    f"Stored embedding {result} for {project_id}/{content_type}"
                )
                return result
        except Exception as e:
    ...
    async def search_similar(
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                logger.debug(
                    f"Found {len(results)} similar embeddings for {project_id}"
                )
                return results
        except Exception as e:
    ...
    async def get_context_for_agent(
        ...
        results = await self.search_similar(
            project_id, query_embedding, top_k, "agent_output"
        )
        ...
        for r in results:
            ...
            if r["distance"] < 0.5:
                ...
                context_parts.append(
                    f"[Previous: {metadata.get('agent_name', 'unknown')}]\n{r['content'][:500]}"
                )
