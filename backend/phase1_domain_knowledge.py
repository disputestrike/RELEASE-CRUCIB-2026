...
class DomainKnowledgeBase:
    ...
    async def _init_engineering_domain(self):
        ...
        ontology = DomainOntology(
            domain="engineering",
            concepts=concepts,
            relationships={"system": ["subsystem", "component", "interface"]},
            hierarchies={
...
class DomainAwareAgent:
    ...
    def __init__(
        self, agent_name: str, domain: str, knowledge_base: DomainKnowledgeBase
    ):
    ...
    async def validate_output(self, output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validation_result = self.knowledge_base.validate_against_domain(
            self.domain, json.dumps(output)
        )

        if not validation_result["compliant"]:
            logger.warning(
                f"Domain violations detected: {validation_result['violations']}"
            )
...
if __name__ == "__main__":
    ...
    print(
        "Supports: Medical, Legal, Financial, Physics, Engineering, Compliance, Security"
    )
