...
class VisionProcessor:
    ...
    async def process_video(self, media: MediaInput, sample_rate: int = 1) -> List[VisionInsight]:
        """
        Process a video.
...
class SensorProcessor:
    ...
    def process_sensor_stream(self, readings: List[SensorReading]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "anomalies_detected": len([a for a in analyses if a["status"] == "anomalous"]),
            "avg_anomaly_score": sum(a["anomaly_score"] for a in analyses) / len(analyses) if analyses else 0
        }
...
class DiagramProcessor:
    ...
    def _infer_diagram_type(self, shapes: List[Dict[str, Any]], 
...
class MultiModalUnderstanding:
    ...
    async def process_multimodal_input(self, media_inputs: List[MediaInput]) -> Dict[str, Any]:
        """
        ...
        for media in media_inputs:
            if media.media_type == MediaType.IMAGE.value:
                ...
                insights_by_type.setdefault("vision", []).extend([i.to_dict() for i in insights])
    ...
    def _synthesize_insights(self, insights_by_type: Dict[str, List[Any]]) -> Dict[str, Any]:
        ...
        if "audio" in insights_by_type:
            synthesis["key_findings"].append("Audio transcription and analysis complete")
