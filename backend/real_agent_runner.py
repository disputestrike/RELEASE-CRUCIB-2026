...
async def run_legacy_file_tool_writes(
    ...
    """
    P3 — Legacy four canonical writes (App, server, schema, tests) via FileAgent.
    Used when ``CRUCIBAI_ASSEMBLY_V2`` is opted out or when assembly V2 raises.
    """
        ...
        try:
            ...
            if r.get("success"):
                ...
                if r.get("success"):
                    ...
                    errors.append(str(r.get("error", "write failed")))
    ...
    test_code = extract_code(
        test_agent.get("output") or test_agent.get("result") or test_agent.get("code"),
        "tests/test_basic.py",
    )
    if test_code:
        try:
