...
async def ai_chat(
    ...
    from server import (
        _call_llm_with_fallback, _get_model_chain, get_workspace_api_keys,
        _effective_api_keys, _user_credits, MIN_CREDITS_FOR_LLM,
        stub_build_enabled, is_real_agent_only, chat_llm_available,
        REAL_AGENT_NO_LLM_KEYS_DETAIL, _stub_detect_build_kind, stub_multifile_markdown,
        _merge_prior_turns_into_message, _build_chat_system_prompt_for_request,
        _needs_live_data, _fetch_search_context, CHAT_WITH_SEARCH_SYSTEM,
        _extract_pdf_text_from_b64, _is_conversational_message, _speed_from_plan,
        _ensure_credit_balance, _tokens_to_credits, screen_user_content,
    )
...
async def ai_chat_history(session_id: str, user: dict = Depends(_get_auth())):
    ...
    if db is None:
        return {"history": []}
    try:
        cursor = db.chat_history.find({"session_id": session_id, "user_id": user["id"]}).sort("created_at", 1)
...
async def ai_chat_stream(
    ...
    from server import (
        _call_llm_with_fallback, _get_model_chain, get_workspace_api_keys,
        _effective_api_keys, _user_credits, MIN_CREDITS_FOR_LLM,
        is_real_agent_only, chat_llm_available, REAL_AGENT_NO_LLM_KEYS_DETAIL,
        screen_user_content, _merge_prior_turns_into_message,
    )
    ...
    async def generate():
        try:
            combined = _merge_prior_turns_into_message((data.message or "").strip(), data.prior_turns)
            model_chain = _get_model_chain(data.model or "auto", combined, effective_keys=effective_stream)
            response, model_used = await _call_llm_with_fallback(
                message=combined,
            ...
            for i, word in enumerate(words):
                ...
                yield _json.dumps({"type": "chunk", "content": chunk, "model": model_used}) + "\n"
...
async def iterative_build(data: ChatMessage, user: dict = Depends(_get_authenticated_or_api_user())):
    """Iterative AI build with streaming phases."""
    from server import (
        _call_llm_with_fallback, _get_model_chain, get_workspace_api_keys,
        _effective_api_keys, _user_credits, MIN_CREDITS_FOR_LLM,
        stub_build_enabled, _stub_detect_build_kind, stub_multifile_markdown,
        screen_user_content,
    )
    import json as _json
    if user and not user.get("public_api") and _user_credits(user) < MIN_CREDITS_FOR_LLM:
    ...
    async def generate():
        try:
            if stub_build_enabled():
                ...
                yield _json.dumps({"type": "phase", "phase": "complete", "files": {"App.js": code}}) + "\n"
                return
            user_keys = await get_workspace_api_keys(user)
            effective = _effective_api_keys(user_keys)
            model_chain = _get_model_chain("auto", data.message or "", effective_keys=effective)
            response, model_used = await _call_llm_with_fallback(
            ...
            yield _json.dumps({"type": "phase", "phase": "complete", "content": response, "model": model_used}) + "\n"
...
async def validate_and_fix(data: ValidateAndFixBody, user: dict = Depends(_get_authenticated_or_api_user())):
    """Validate code with LLM; if issues found, run auto-fix and return fixed code."""
    from server import (
        _call_llm_with_fallback, _get_model_chain, get_workspace_api_keys, _effective_api_keys,
    )
    try:
        user_keys = await get_workspace_api_keys(user)
        effective = _effective_api_keys(user_keys)
