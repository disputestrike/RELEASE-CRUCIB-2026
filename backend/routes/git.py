...
async def git_status(project_id: Optional[str] = Query(None), user: dict = Depends(_get_auth())):
...
async def git_stage(project_id: Optional[str] = Query(None), file_path: str = Query(...), user: dict = Depends(_get_auth())):
    from git_integration import git_manager
    path = await _resolve_ws(project_id, user)
...
async def git_commit(project_id: Optional[str] = Query(None), body: GitCommitRequest = None, user: dict = Depends(_get_auth())):
...
async def git_branches(project_id: Optional[str] = Query(None), user: dict = Depends(_get_auth())):
...
async def git_merge(project_id: Optional[str] = Query(None), branch: str = Query(...), user: dict = Depends(_get_auth())):
...
async def git_resolve_conflict(project_id: Optional[str] = Query(None), body: GitResolveRequest = None, user: dict = Depends(_get_auth())):
