...
async def ide_debug_start(project_id: str = Query(...), user: dict = Depends(_get_auth())):
...
async def ide_debug_set_breakpoint(session_id: str, body: IDEBreakpointRequest, user: dict = Depends(_get_auth())):
...
async def ide_debug_remove_breakpoint(session_id: str, breakpoint_id: str, user: dict = Depends(_get_auth())):
    """Remove a breakpoint."""
    from ide_features import debugger_manager
    try:
...
async def ide_profiler_start(project_id: str = Query(...), user: dict = Depends(_get_auth())):
...
async def ide_profiler_stop(session_id: str = Query(...), user: dict = Depends(_get_auth())):
...
async def ide_lint(project_id: str = Query(...), file_path: Optional[str] = None, code: Optional[str] = None, user: dict = Depends(_get_auth())):
