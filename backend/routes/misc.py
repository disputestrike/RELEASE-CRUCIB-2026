...
PROMPT_TEMPLATES = [
    ...
    {
        "id": "paypal-saas",
        "name": "PayPal subscription SaaS",
        "prompt": "Build a SaaS landing page with pricing cards and PayPal checkout integration for subscription payments. React and Tailwind.",
        "category": "app",
    },
    {
        "id": "todo",
...
@router.post("/ai/inject-paypal")
async def inject_paypal(
    data: InjectPaymentBody, user: dict = Depends(_get_authenticated_or_api_user())
):
    """Inject a PayPal checkout client into React code. Uses workspace model keys when set."""
    (
        _call_llm_with_fallback,
        _get_model_chain,
        get_workspace_api_keys,
    ...
    user_keys = await get_workspace_api_keys(user)
    effective = _effective_api_keys(user_keys)
    model_chain = _get_model_chain("auto", "paypal checkout", effective_keys=effective)
    prompt = f"""Add a PayPal checkout flow to this React code.
Target: {data.target}.

Requirements:
- Use the PayPal JS SDK.
- Create orders through /api/billing/create-order.
- Capture approved orders through /api/billing/capture-order.
- Do not use Stripe packages, Stripe.js, STRIPE_* env vars, or fake checkout URLs.
- If server credentials are missing, render a clear disabled state that says PayPal credentials are required.
