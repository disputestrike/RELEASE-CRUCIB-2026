...
def navier_stokes_2d(t, y, nu=0.01, Lx=1.0, Ly=1.0):
    ...
    du_dt = (
        -u * (du_dx := 0.1) - v * (du_dy := 0.1) - (dp_dx := 0.1) + nu * (d2u := 0.01)
    )
    dv_dt = (
        -u * (dv_dx := 0.1) - v * (dv_dy := 0.1) - (dp_dy := 0.1) + nu * (d2v := 0.01)
    )
...
def traveling_salesman_np():
    ...
    print(
        f"Time to check all routes (brute force): O(n!) = {math.factorial(n_cities)} operations"
    )
