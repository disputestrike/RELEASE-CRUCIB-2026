...
class SpeedTierRouter:
    ...
    def validate_speed_tier_access(plan: str, speed_tier: str) -> tuple[bool, str]:
        ...
        if speed_tier not in allowed_speeds:
            if speed_tier == "pro":
                ...
                return False, f"Speed tier '{speed_tier}' is not available on your plan."
    ...
    def route_request(plan: str, speed_tier: str, base_tokens: int) -> dict:
        ...
        """
        # Validate access
        is_valid, error_msg = SpeedTierRouter.validate_speed_tier_access(plan, speed_tier)
        if not is_valid:
            return {
                "valid": False,
                "error": error_msg,
        ...
        adjusted_tokens = SpeedTierRouter.apply_token_multiplier(base_tokens, speed_tier)
