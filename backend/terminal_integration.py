...
def _run_command_sync(
    cwd: Path, shell: str, command: str, timeout_sec: int = TERMINAL_CMD_TIMEOUT
) -> tuple[int, str, str]:
...
def _command_blocked_by_policy(command: str) -> bool:
    ...
    if "|" in normalized and any(
        token in normalized for token in (" sh", " bash", "powershell", "pwsh")
    ):
        return True
...
class TerminalManager:
    ...
    def _record_audit(
        self, session: Optional[TerminalSession], command: str, result: Dict[str, Any]
    ) -> None:
    ...
    async def execute(
        ...
        session_id: str,
        command: str,
        timeout: int = TERMINAL_CMD_TIMEOUT,
        user_id: str = "",
    ) -> Dict[str, Any]:
        ...
        if _command_blocked_by_policy(command):
            result = {
                "returncode": -1,
                "stdout": "",
                "stderr": "Command blocked by terminal policy",
            }
        ...
        if not path.exists():
            result = {
                "returncode": -1,
                "stdout": "",
                "stderr": "Project path does not exist",
            }
            self._record_audit(session, command, result)
            return result
        returncode, stdout, stderr = await asyncio.to_thread(
            _run_command_sync, path, session.shell, command, timeout
        )
