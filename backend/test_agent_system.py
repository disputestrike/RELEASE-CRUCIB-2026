...
class MockCursor:
    ...
    async def to_list(self, length):
        ...
        if self.sort_order:
            ...
            data = sorted(
                data, key=lambda x: x.get(field, 0), reverse=(direction == -1)
            )
...
class TestPerformanceTracker:
    ...
    async def test_record_metric(self):
        ...
        await tracker.record_metric(
            agent_name="TestAgent", metric_name="execution_time_ms", value=150.5
        )

        # Verify it was stored
        metrics = (
            await db["agent_performance"].find({"agent_name": "TestAgent"}).to_list(10)
        )
    ...
    async def test_metrics_summary(self):
        ...
        summary = await tracker.get_metrics_summary(
            agent_name="TestAgent", metric_name="execution_time_ms"
        )
...
class TestBaseAgent:
    ...
    async def test_agent_execution_with_learning(self):
        ...
        executions = (
            await db["agent_memory"].find({"agent_name": "TestAgent"}).to_list(10)
        )
