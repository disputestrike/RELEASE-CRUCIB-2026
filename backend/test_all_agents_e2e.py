...
class AgentE2ETest:
    ...
    async def test_agent(self, agent_name: str, test_input: Dict[str, Any]) -> Dict[str, Any]:
        """Test a single agent"""
    ...
    async def run_all_e2e_tests(self) -> Dict[str, Any]:
        ...
        print(f"  Success Rate: {(self.results['passed'] / max(self.results['total_agents'], 1)) * 100:.1f}%")
...
class LoadTest:
    ...
    async def load_test_agent(
        ...
        if self.results["latencies"]:
            stats = {
                ...
                "p95_latency_ms": sorted(self.results["latencies"])[int(len(self.results["latencies"]) * 0.95)],
            }
        else:
    ...
    async def run_load_tests(self, num_requests_per_agent: int = 50) -> Dict[str, Any]:
        ...
        print(f"  Failed: {self.results['failed']}")
        print(f"  Success Rate: {(self.results['successful'] / max(self.results['total_requests'], 1)) * 100:.1f}%")
        ...
        if self.results["latencies"]:
            ...
            print(f"  P95: {sorted(self.results['latencies'])[int(len(self.results['latencies']) * 0.95)]:.0f}ms")
...
class StressTest:
    ...
    async def run_stress_tests(self) -> Dict[str, Any]:
        ...
        for agent_name in list(self.agents.keys())[:5]:
            result = await self.stress_test_agent(agent_name, max_concurrent=50, duration_seconds=20)
...
async def main():
    ...
    agents = {f"Agent_{i}": MockAgent(f"Agent_{i}") for i in range(10)}  # 10 mock agents
