...
def test_observability_intent_keywords():
    ...
    assert observability_intent(
        {"goal": "Grafana dashboards with structured JSON logs"}
    )
...
def test_multiregion_terraform_intent():
    ...
    assert multiregion_terraform_intent(
        "Terraform for GKE in europe-west1 and failover"
    )
...
def test_verify_security_observability_and_terraform_dirs():
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(
            os.path.join(d, "terraform", "multiregion_sketch", "main.tf"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write("# stub\n")
