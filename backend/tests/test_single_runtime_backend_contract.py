...
@pytest.mark.asyncio
async def test_create_job_defaults_to_single_runtime_without_legacy_steps(monkeypatch):
    ...
    assert result["plan"]["engine"] == "single_tool_runtime"
    assert result["plan"]["legacy_job_steps"] is False
    assert state.steps_created == []
    assert any(event["event_type"] == "runtime_backend_selected" for event in state.events)


@pytest.mark.asyncio
async def test_steps_endpoint_filters_legacy_agents_when_tool_runtime_is_active():
    state = _RuntimeState()
    state.steps = [
        {"id": "old-1", "step_key": "old.planner", "agent_name": "Planner", "phase": "orchestration"},
        {"id": "new-1", "step_key": "runtime.proof", "agent_name": "Runtime", "phase": "runtime"},
    ]
    state.events = [{"event_type": "pipeline_started", "payload": {"engine": "single_tool_runtime"}}]
