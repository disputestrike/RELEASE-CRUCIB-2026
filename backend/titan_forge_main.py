...
from sqlalchemy import create_engine, Column, String, Integer, Boolean, DateTime, JSON, LargeBinary, ForeignKey, event, text
...
class Role(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid_lib.uuid4)
    org_id = Column(UUID(as_uuid=True), ForeignKey("organizations.id"), nullable=True)  # NULL = global role
    name = Column(String, nullable=False)  # e.g., "org_admin", "operator"
    permissions = Column(JSON, nullable=False, default=dict)  # e.g., {"quote_approve": True, "policy_enforce": True}
    created_at = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
...
class KeyWrapper(Base):
    ...
    rotation_timestamp = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
    ...
    def __init__(self):
        ...
        try:
            ...
            if len(self.master_key) < 32:
                ...
                self.master_key = (self.master_key * (32 // len(self.master_key) + 1))[:32]
...
async def get_current_org(request: Request) -> str:
    ...
    if not org_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Org context missing")
    return org_id
...
def init_db():
    ...
    try:
        ...
        test_org = db.query(Organization).filter(Organization.name == "Test Org").first()
        if not test_org:
            ...
            # Create roles
            admin_role = Role(org_id=test_org.id, name="org_admin", permissions={"quote_approve": True, "policy_enforce": True})
            operator_role = Role(org_id=test_org.id, name="operator", permissions={"quote_view": True})
            db.add(admin_role)
