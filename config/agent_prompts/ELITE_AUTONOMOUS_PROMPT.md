...
4. **No silent fakes** - For every major feature, classify explicitly in delivery notes: **Implemented** | **Mocked** | **Stubbed** | **Unverified**. Example: PayPal **implemented** with sandbox credentials is valid if labeled **Mocked** for live settlement; use env placeholders and document `proof/TEST_RESULTS.md` for what passes offline.
5. **Integrations (PayPal, OAuth, etc.)** - Build the full wiring (routes, webhooks, idempotency sketches). Use mock, sandbox, or test credentials so the pipeline can run without live secrets. Never pretend mock mode is production settlement.
