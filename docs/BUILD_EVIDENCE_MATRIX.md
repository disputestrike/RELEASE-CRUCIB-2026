...
|---|---|---|
| Automatic BIV retry | Final BIV failure now triggers a bounded `build_integrity_validator_repair_attempt` using the existing repair brain, reruns BIV, and exposes `retry_route.agent_groups` for targeted DAG routing. | Full live DAG node-level requeue by category is still deeper work. |
...
```bash
python -m py_compile backend/orchestration/build_integrity_validator.py backend/orchestration/import_doctor.py backend/automation/executor.py backend/orchestration/auto_runner.py scripts/verify_claim_evidence_parity.py
