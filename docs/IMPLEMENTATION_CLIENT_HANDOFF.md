...
| P3 | Legacy four-file path / no duplicate writes with V2 | **`run_legacy_file_tool_writes`** in `backend/real_agent_runner.py` (standalone module removed). Used only when V2 is opted out (`CRUCIBAI_ASSEMBLY_V2=0|false|no|off`) or when assembly V2 raises (fallback). **Default-on V2** avoids duplicate writes in normal runs. **Regression:** `backend/tests/test_legacy_file_tool_writes.py`. |
