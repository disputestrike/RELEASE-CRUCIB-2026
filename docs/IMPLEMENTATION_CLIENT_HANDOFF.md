...
| `CRUCIBAI_ASSEMBLY_V2` | **Default: on** (multi-file assembly). Set to `0`, `false`, `no`, or `off` to use the legacy four-file writer (`real_agent_runner.run_legacy_file_tool_writes`). |
...
| `proof_bundle_generator.py` | **`workspace_meta/`** in the on-disk proof bundle: copies `proof_index.json`, `artifact_manifest.json`, `run_manifest.json`, `seal.json`, `path_last_writer.json`, `merge_map.json` (when present). |
...
| P3 | Thin legacy shim / remove duplicate writes | **No separate shim file:** legacy four-file path is **`run_legacy_file_tool_writes`** in `real_agent_runner.py` — used only when V2 is opted out (`CRUCIBAI_ASSEMBLY_V2=0|false|no|off`) or when assembly V2 raises (fallback). **Default-on V2** avoids duplicate writes in normal runs. **Regression:** `backend/tests/test_legacy_file_tool_writes.py`. |
