  ...
  useEffect(() => {
    ...
    if (errorFromUrl) {
      setError(
        ...
        errorFromUrl === 'google_failed' ? 'Google sign-in failed. Add your callback URL to Google Cloud Console → Credentials → Authorized redirect URIs (see server logs for the exact URL).' :
                      ...
                      >
                        {passwordChecks.map((check) => (
                          <div key={check.label} className="flex items-center gap-1.5">
                            <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center ${check.met ? 'bg-[#1A1A1A]' : 'bg-gray-200'} transition-colors`}>
                              {check.met && <Check className="w-2.5 h-2.5 text-white" />}
                            </div>
                            <span className={`text-xs ${check.met ? 'text-[#1A1A1A]' : 'text-gray-400'} transition-colors`}>
                              {check.label}
                            </span>
                          </div>
                        ))}
