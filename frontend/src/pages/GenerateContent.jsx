...
import { formatModelUsageLine, isNonBillableModel } from "../utils/modelUsageLabel";
...
export default function GenerateContent() {
  ...
  return (
    <div className="min-h-[60vh] p-6 max-w-4xl mx-auto">
      ...
      <div className="flex gap-2 border-b border-gray-200 mb-6">
        {tabs.map((t) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-t font-medium text-sm ${activeTab === t.id ? "bg-[#1A1A1A] text-white" : "bg-[#F5F5F4] text-[#1A1A1A] hover:bg-[#EBE8E2]"}`}
          >
      ...
      <div className="space-y-4">
        <div>
          ...
          <textarea
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1A1A1A]/20 focus:border-[#1A1A1A] min-h-[80px]"
            rows={3}
          />
        </div>
        <div className="flex flex-wrap items-center gap-4">
          ...
          <button
            ...
            disabled={loading}
            className="px-4 py-2 bg-[#1A1A1A] text-white rounded-lg font-medium text-sm hover:bg-[#333] disabled:opacity-50 flex items-center gap-2"
          >
      ...
      {modelUsed && (
        <p
          className={`text-xs mt-1 ${isNonBillableModel(modelUsed) ? "text-neutral-800 bg-neutral-100 border border-neutral-200 rounded px-2 py-1" : "text-gray-500"}`}
        >
      ...
      {content && (
        <div className="mt-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={download} className="flex items-center gap-1 text-sm text-[#1A1A1A] hover:underline">
