...
export default function PaymentsWizard() {
  ...
  const handleInjectPayPal = async () => {
    if (!token) {
      navigate('/app');
      return;
    }
    ...
    <div className="min-h-screen bg-[#FAFAF8] text-[#1A1A1A] p-6">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold">Add payments (PayPal)</h1>
            <p className="text-zinc-400">Wizard to add PayPal Checkout to your app</p>
          </div>
        </div>
            ...
            <>
              <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/50">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="font-semibold mb-2">Step 1: Get your PayPal keys</h2>
                    <p className="text-sm text-zinc-400">Create a PayPal app, then add PAYPAL_CLIENT_ID, PAYPAL_CLIENT_SECRET, and PAYPAL_MODE to your project env.</p>
                  </div>
                </div>
              </div>
            ...
            <>
              <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/50">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="font-semibold mb-2">Step 2: Inject PayPal into your code</h2>
                    <p className="text-sm text-zinc-400">We'll add PayPal Checkout to your React app. Run this from the Workspace with your current App.js, or use the sample below.</p>
                  </div>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setStep(1)} className="px-4 py-2 rounded-lg bg-zinc-800 text-zinc-300">Back</button>
                <button onClick={handleInjectPayPal} disabled={injecting} className="px-4 py-2 rounded-lg bg-[#F3F1ED] text-[#1A1A1A] hover:bg-[#333] disabled:opacity-50">
                  {injecting ? 'Injecting...' : 'Inject PayPal'}
                </button>
              </div>
            </>
          )}
              ...
              <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/50">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-400 mb-3">Copy the code below into your App.js in the Workspace, or start from a template that includes PayPal.</p>
