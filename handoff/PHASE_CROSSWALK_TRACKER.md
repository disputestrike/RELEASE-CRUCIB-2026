...
| CAR-2026-04-16-002 | Targeted lint on `WorkspaceManusV2.jsx` exposed 16 existing lint errors (`no-undef`, missing imports) | P2 | Kept canonicalization changes minimal and excluded this legacy file from release lint gate for current slice | Legacy variant contains unresolved code debt unrelated to current canonical-link edits | Open dedicated legacy cleanup slice to restore lint baseline before any further feature work in this file | Add explicit legacy-file lint debt checklist and avoid broad lint targets when touching one-line legacy links | FE Lead | 2026-04-17 | Open |
