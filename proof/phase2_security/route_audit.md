...
| backend/server.py | 9257 | POST | `/vibecoding/detect-frameworks` | safe as optional | must require project ownership when project_id is supplied; code enforces auth and user_id lookup |
| backend/server.py | 9610 | GET | `/skills/marketplace` | safe as optional | safe as optional: public marketplace listing plus own user skills when authenticated |
